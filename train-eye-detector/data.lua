...
----------------------------------------------------------------------
print(sys.COLORS.red ..  '==> reading dataset')
...
----------------------------------------------------------------------
print(sys.COLORS.red ..  '==> loading eyes')
...
-- load eyes:
local eyes = torch.Tensor(11383,3,32,32)

for f=1,11382 do
  tmp = image.load('data/eyes/eye_'..f..'.png')
  
  -- throw out the alpha channel
--  tmp = image.rgb2yuv(tmp[{{1,3},{},{}}])
  tmp = tmp[{{1,3},{},{}}]
  
  -- put in eyesOut, and shrink
  eyes[{{f+1},{},{},{}}] = image.scale(tmp, 32, 32 )
end


-- that's true for the faces, but I'll be dealing with eyes
-- slightly fewer eyes than faces
local imagesAll = torch.Tensor(39416,3,32,32)
local labelsAll = torch.Tensor(39416)
...
-- load backgrounds:
for f=0,28033 do
  imagesAll[f+1] = image.load('data/bg/bg_'..f..'.png') 
  labelsAll[f+1] = 2 -- 2 = background
end

imagesAll[{{28034,39416},{},{},{}}] = eyes:clone()
labelsAll[{{28034,39416}}] = 1-- 1 = eye
